...
$( document ).ready(function() {
    promos.forEach(function(item) {
        ...
        "<a data-product-id='"+item.productId+"' class='btn mt-4 d-block bg-color-yellow text-center p-1 text-uppercase seeProductInfo'>Ver Producto</a>"+
        "</div>"+
        "</div>"; 
        $('#zonaPromotions').append(query);
    });
    ...
    $(".seeProductInfo").on('click', function(event){
       ...
       $.each(promos, function(index, product) {
      ...
      let contentDynamic = `<img src="${productFinded.img}" alt="Harmonics music logo"  style="width:100px;heigth:auto" >
      <a class='fs-3 color-yellow fw-bold text-decoration-none'>${productFinded.productPrize}</a>`;
      var dataInfo = document.createElement("div");
       dataInfo.innerHTML =contentDynamic; 

      swal({
        title: productFinded.productName,
        text: productFinded.productDesc,
